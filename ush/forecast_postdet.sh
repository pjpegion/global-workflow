...
MOM6_out()
{
  ...
  # ------------------------------------------------------
	      ...
	      # PJP $NCP -p $ocnfile $COMOUT/ocn_${p_date}.nc
	      status=$?
...
CICE_out()
{
	...
	for fhr in $fhrlst; do
	  ...
	  if [[ 10#$fhr -eq 0 ]]; then
	    ...
	    status=$?
	    [[ $status -ne 0 ]] && exit $status
            echo "fhr is 0, only copying ice initial conditions... exiting"          #fi
          else    
            $NCP -p $DATA/history/iceh_`printf "%0.2d" $FHOUT`h.${YYYY}-${MM}-${DD}-`printf "%5.5d" ${SS}`.nc $COMOUT/ice$VDATE.$ENSMEM.$IDATE.nc    
	  # for daily output  $NCP -p $DATA/history/iceh.${YYYY}-${MM}-${DD}.nc $COMOUT/iceh_${VDATE}.nc
            status=$?    
            [[ $status -ne 0 ]] && exit $status    
         fi
